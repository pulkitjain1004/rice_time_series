load("C:/Users/Pulkit Jain/Desktop/ISEN__/626 Time Series/Project/Final/dat0705.RData")
...
x <- tsdf[,-3]
plot(x)
varselect <- VARselect(x, lag.max = 10, type="both")$criteria
varselect <- as.data.frame(varselect)
write.csv(varselect, file="err.csv")

err_table <- read.csv("C:/Users/Pulkit Jain/Desktop/ISEN__/626 Time Series/Project/Final/err.csv")
plot(window(err_table[,1], type="b", ylim=c(-100,100)))
lines(window(err_table[,2], type="b"))
plot(err_table[,3], type="b", ylim=c(-41,-34))
lines(err_table[,1], type="b", col="red")
legend("topleft", col=c("black", "red"), c("BIC","AIC"))


# try either order 1 or order 2

summary(fit2 <- VAR(x, p=2, type="both"))
acf(resid(fit2))
summary(fit1 <- VAR(x, p=1, type="both"))
acf(resid(fit1))
